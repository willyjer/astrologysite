...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { validateAndSanitizeBirthData, validateBirthDataRanges } = await import('../../lib/inputSanitization');
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Astrology API error: ${response.status}`, details: errorText },
